...
function findConformanceDescriptor(file) {
  ...
  if (descriptor.type != null && !t.isObjectExpression(descriptor)) {
    throw new Error(
      `Expected an object expression as a descriptor but found ${descriptor.type}`,
    );
  }
...
function getRefInstance(valueNode) {
  if (
    !babel.types.isMemberExpression(valueNode) &&
    valueNode.name !== 'Object'
  ) {
  ...
  switch (valueNode.object.name) {
    ...
    default:
      throw new Error(
        `Unrecognized member expression starting with '${valueNode.object.name}'`,
      );
  }
}
