...
export const isPrimitive = (prop: PropType | { name: string }): prop is PropType$Primitive =>
	[
		'string',
		'int',
		'float',
...
export const isObjectSignature = (prop: PropType): prop is PropType$ObjectSignature =>
	prop.name === 'signature' && prop.type === 'object';

export const isFunctionSignature = (prop: PropType): prop is PropType$FunctionSignature =>
...
export const isCallback = (propName: string, prop: PropType): prop is PropType$Callback =>
	isFunc(prop) && isCallBackName(propName);

export const isUnionEnum = (union: PropType) =>
	isUnion(union) && union.value != null
		? union.value.reduce(
				(literalsOnly, unionProp) => (!literalsOnly ? literalsOnly : unionProp.name === 'literal'),
				true,
			)
		: false;
