...
const parseInit = () => {
	...
	components.forEach((component) => {
		...
		Fs.writeFileSync(Path.join(outputDirectory, 'reason', `MaterialUi_${component.name}.re`), rendered);
	});

	// Copy fixed modules
	const items = Fs.readdirSync(Path.join(__dirname, 'fixed-modules'));
	const itemsFiltered = items.filter(
		(item) => item.lastIndexOf('.re') === item.length - 3 || item.lastIndexOf('.js') === item.length - 3,
	);
	itemsFiltered.forEach((item) => {
		Fs.copyFileSync(Path.join(__dirname, 'fixed-modules', item), Path.join(outputDirectory, 'reason', item));
	});

	Fs.writeFileSync(Path.join(outputDirectory, 'reason', 'MaterialUi_Theme.re'), RenderTheme.theme);
	Fs.writeFileSync(Path.join(outputDirectory, 'reason', 'MaterialUi_ThemeOptions.re'), RenderTheme.themeOptions);
	...
	Fs.writeFileSync(
		...
		`
        ${components
			.map((component) => (component != null ? `module ${component.name} = MaterialUi_${component.name};` : ''))
			.join('\n')}
