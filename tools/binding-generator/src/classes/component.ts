...
class Component {
	...
	public render() {
		...
		return `
            ...
            ${hasProps ? `[@bs.obj] external makePropsMui : (${this.renderSection('MakeProps')} unit) => _ = "";` : ''}

            let makeProps = (
                ${this.renderSection('Make')}
                ()
            ) => 
                ${!hasProps ? 'Js.Obj.empty()' : `makePropsMui(${this.renderSection('WrapJs')} ())`};

            [@bs.module "${this._component.importPath}"] external make : React.component('a) = "${this._component
			.importName || 'default'}";
        `;
	}
}
