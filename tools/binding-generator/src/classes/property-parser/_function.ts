    ...
    return class FunctionParser extends Base {
        ...
        public executeParse() {
            ...
            if (this._signature.arguments.length) {
                ...
                if (args.length) {
                    ...
                    if (shouldLabel) {
                        argString = `(${
                            args.map(arg => `~${arg.name}: ${arg.type}${!arg.required ? '=?' : ''}`).join(', ')
                        }, unit)`;
                    }
                    else {
                        argString = `(${
        ...
        private parseArguments() {
            ...
            argumentSignatures.forEach(arg => {
                ...
                const argumentParser = ResolveArgument(name, arg.required || true, arg.type, this._property);
                if (argumentParser) {
                    args.push({
                        name,
        ...
        private parseReturn() {
            const returnParser = ResolveArgument(`${this._property.name}Return`, true, this._signature.return, this._property);
            if (returnParser) {
                if (returnParser.valid) {
                    return returnParser.reasonType;
                }
                else {
                    Console.error(`_function: Parser returned false for argument ${Console.colors.green}${this.property.component.name}/${this.property.name}/${this._property.name}Return/${this._signature.return.name}${Console.colors.reset}`);
                    return generateAny();
                }
            }
            else {
