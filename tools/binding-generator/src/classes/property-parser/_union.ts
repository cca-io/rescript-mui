		...
		public executeParse() {
				...
				if (type != null) {
					if (Identify.isPrimitive(type) && type.name !== 'array') {
						...
						switch (pType) {
							...
							default:
								Console.error(
									`_union: Unhandled primitive type ${Console.colors.red}${JSON.stringify(
										unionProp.property.signature,
									)}${Console.colors.yellow} in Union ${Console.colors.red}${this.property.name}`,
								);
						}
					} else if (Identify.isPrimitive(type) && type.name === 'array') {
						...
						Console.error(
							`_union: Unhandled complex type ${Console.colors.red}${JSON.stringify(unionProp)}${Console
								.colors.yellow} in Union ${Console.colors.red}${this.property.name}`,
						);
					}
				}
			});
			...
			if (this.property.signature.required) {
				if (this._hasEnum || this._hasEnumArray) {
                    ...
                    (fun
                        ${enums
							.map((x) => `| \`Enum(v) => MaterialUi_Helpers.unwrapValue(\`String(${x}ToJs(v)))`)
							.join('\n')}
                        ${enumArrays
							.map(
								(x) =>
