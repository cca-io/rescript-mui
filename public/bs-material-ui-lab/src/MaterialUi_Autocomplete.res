...
let element = (v: React.element) => Any(v);
            ...
            @react.component @bs.module("@material-ui/lab") external make : (~autoComplete: option<bool>=?,
~autoHighlight: option<bool>=?,
~autoSelect: option<bool>=?,
~blurOnSelect: option<BlurOnSelect.t>=?,
~\"ChipProps": option<Js.Dict.t<MaterialUi_Types.any>>=?,
...
~\"ListboxComponent": option<ListboxComponent.t>=?,
~\"ListboxProps": option<Js.Dict.t<MaterialUi_Types.any>>=?,
~loading: option<bool>=?,
...
~onHighlightChange: option<(Js.Dict.t<MaterialUi_Types.any>, string) => unit>=?,
~onInputChange: option<(Js.Dict.t<MaterialUi_Types.any>, string, string) => unit>=?,
~onOpen: option<(ReactEvent.Synthetic.t) => unit>=?,
~\"open": option<bool>=?,
~openOnFocus: option<bool>=?,
~openText: option<string>=?,
~options: array<MaterialUi_Types.any>,
~\"PaperComponent": option<PaperComponent.t>=?,
...
~renderOption: option<(Js.Dict.t<MaterialUi_Types.any>) => MaterialUi_Types.any>=?,
...
~key: option<string>=?,
~ref: option<ReactDOMRe.domRef>=?,) => React.element = "Autocomplete";
