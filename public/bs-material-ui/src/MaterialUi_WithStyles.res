...
module WithStylesSafe = (S: WithStylesSafeTemplate) => {
  ...
  let withStylesWithTheme: (MaterialUi_Theme.t => S.classRecordJs) => React.component<
    'a,
  > = MaterialUi_WithStyles_Helper.createStyled
  ...
  let useStyles = () => {
    let stylesHook = React.useMemo0(() =>
      switch S.classes {
      | Record(record) => makeStyles(record->S.classRecordToJs)
      | ThemeFunc(func) => makeStylesWithTheme(theme => func(theme)->S.classRecordToJs)
      }
    )
  ...
  module Styled = {
    let styled = switch S.classes {
    | Record(record) => withStyles(record->S.classRecordToJs)
    | ThemeFunc(func) => withStylesWithTheme(theme => func(theme)->S.classRecordToJs)
    }

    @react.component
    let make = (~children: {"classes": S.classRecordStringsJs} => React.element) =>
      React.createElement(styled, {"children": children})
  }

  @react.component
  let make = (~children: S.classRecordStrings => React.element) =>
    <Styled> ...{classes => children(classes["classes"]->S.classRecordStringsFromJs)} </Styled>
}

let createStyled: Js.Dict.t<ReactDOM.Style.t> => React.component<
  'a,
> = MaterialUi_WithStyles_Helper.createStyled
let createStyledWithTheme: (MaterialUi_Theme.t => Js.Dict.t<ReactDOM.Style.t>) => React.component<
  'a,
> = MaterialUi_WithStyles_Helper.createStyled
...
let make = (
  ...
  let generateDict = (lst: list<style>) => {
    ...
    lst->Belt.List.forEach(style => Js.Dict.set(classDict, style.name, style.styles))
    classDict
  }

  let styled = switch (classes, classesWithTheme) {
  | (Some(classList), None) => createStyled(classList->generateDict)
  | (None, Some(func)) => createStyledWithTheme(theme => func(theme)->generateDict)
  | _ => raise(Not_found)
  }
