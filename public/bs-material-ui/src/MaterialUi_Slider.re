...
external makePropsMui:
  (
    ...
    ~className: string=?,
    ~color: string=?,
    ~component: 'union_rf4w=?,
    ~defaultValue: 'union_rv83=?,
    ~disabled: bool=?,
    ~getAriaLabel: 'any_r8t9=?,
    ~getAriaValueText: 'any_rosh=?,
    ~marks: 'union_rc9s=?,
    ~max: 'number_8=?,
    ~min: 'number_s=?,
    ~name: string=?,
    ~onChange: 'any_r3yv=?,
    ~onChangeCommitted: 'any_rw2x=?,
    ~onMouseDown: ReactEvent.Mouse.t => unit=?,
    ~orientation: string=?,
    ~scale: 'genericCallback=?,
    ~step: 'number_k=?,
    ~_ThumbComponent: 'union_rv60=?,
    ~track: string=?,
    ~value: 'union_rd5c=?,
    ~_ValueLabelComponent: 'union_r0e8=?,
    ~valueLabelDisplay: string=?,
    ~valueLabelFormat: 'union_r8in=?,
    ~key: string=?,
...
let makeProps =
    (
      ...
      ~marks: option([ | `Bool(bool) | `Array('any_rpev)])=?,
