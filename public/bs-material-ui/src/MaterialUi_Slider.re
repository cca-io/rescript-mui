...
external makePropsMui:
  (
    ...
    ~className: string=?,
    ~color: string=?,
    ~component: 'union_ra8r=?,
    ~defaultValue: 'union_r2s2=?,
    ~disabled: bool=?,
    ~getAriaLabel: 'any_r4ju=?,
    ~getAriaValueText: 'any_rl5s=?,
    ~marks: 'union_rqj4=?,
    ~max: 'number_3=?,
    ~min: 'number_q=?,
    ~name: string=?,
    ~onChange: 'any_rotg=?,
    ~onChangeCommitted: 'any_r4dk=?,
    ~onMouseDown: ReactEvent.Mouse.t => unit=?,
    ~orientation: string=?,
    ~scale: 'genericCallback=?,
    ~step: 'number_3=?,
    ~_ThumbComponent: 'union_ru6z=?,
    ~track: string=?,
    ~value: 'union_ro1k=?,
    ~_ValueLabelComponent: 'union_r86y=?,
    ~valueLabelDisplay: string=?,
    ~valueLabelFormat: 'union_rqzw=?,
    ~id: string=?,
    ~key: string=?,
...
let makeProps =
    (
      ...
      ~marks: option([ | `Bool(bool) | `Array('any_r2mv)])=?,
