...
external makePropsMui:
  (
    ...
    ~className: string=?,
    ~color: string=?,
    ~component: 'union_rzkd=?,
    ~defaultValue: 'union_rsjl=?,
    ~disabled: bool=?,
    ~getAriaLabel: 'any_rlzg=?,
    ~getAriaValueText: 'any_rqi8=?,
    ~marks: 'union_r3cx=?,
    ~max: 'number_d=?,
    ~min: 'number_d=?,
    ~name: string=?,
    ~onChange: 'any_rgl5=?,
    ~onChangeCommitted: 'any_rk1w=?,
    ~onMouseDown: ReactEvent.Mouse.t => unit=?,
    ~orientation: string=?,
    ~scale: 'genericCallback=?,
    ~step: 'number_l=?,
    ~_ThumbComponent: 'union_rdwc=?,
    ~track: 'any_rqsv=?,
    ~value: 'union_r8jc=?,
    ~_ValueLabelComponent: 'union_rb78=?,
    ~valueLabelDisplay: string=?,
    ~valueLabelFormat: 'union_raxj=?,
    ~id: string=?,
    ~key: string=?,
...
let makeProps =
    (
      ...
      ~marks: option([ | `Bool(bool) | `Array('any_rrlu)])=?,
