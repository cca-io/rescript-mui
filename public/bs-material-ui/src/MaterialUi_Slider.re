...
external makePropsMui:
  (
    ...
    ~className: string=?,
    ~color: string=?,
    ~component: 'union_rjxd=?,
    ~defaultValue: 'union_ra67=?,
    ~disabled: bool=?,
    ~getAriaLabel: 'any_rvm9=?,
    ~getAriaValueText: 'any_r6p6=?,
    ~marks: 'union_rwmf=?,
    ~max: 'number_8=?,
    ~min: 'number_z=?,
    ~name: string=?,
    ~onChange: 'any_ruh7=?,
    ~onChangeCommitted: 'any_r7tw=?,
    ~onMouseDown: ReactEvent.Mouse.t => unit=?,
    ~orientation: string=?,
    ~scale: 'genericCallback=?,
    ~step: 'number_o=?,
    ~_ThumbComponent: 'union_r0vt=?,
    ~track: string=?,
    ~value: 'union_rhi3=?,
    ~_ValueLabelComponent: 'union_rr3l=?,
    ~valueLabelDisplay: string=?,
    ~valueLabelFormat: 'union_risx=?,
    ~key: string=?,
...
let makeProps =
    (
      ...
      ~marks: option([ | `Bool(bool) | `Array('any_rgw0)])=?,
