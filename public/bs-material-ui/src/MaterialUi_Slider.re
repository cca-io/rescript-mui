...
external makePropsMui:
  (
    ...
    ~className: string=?,
    ~color: string=?,
    ~component: 'union_rrmj=?,
    ~defaultValue: 'union_rekf=?,
    ~disabled: bool=?,
    ~getAriaLabel: 'any_rlwy=?,
    ~getAriaValueText: 'any_r0h1=?,
    ~marks: 'union_rbrh=?,
    ~max: 'number_7=?,
    ~min: 'number_n=?,
    ~name: string=?,
    ~onChange: 'any_r507=?,
    ~onChangeCommitted: 'any_rr9c=?,
    ~onMouseDown: ReactEvent.Mouse.t => unit=?,
    ~orientation: string=?,
    ~step: 'number_q=?,
    ~_ThumbComponent: 'union_ra9v=?,
    ~track: string=?,
    ~value: 'union_rcy1=?,
    ~_ValueLabelComponent: 'union_r0np=?,
    ~valueLabelDisplay: string=?,
    ~valueLabelFormat: 'union_r0dk=?,
    ~key: string=?,
...
let makeProps =
    (
      ...
      ~marks: option([ | `Bool(bool) | `Array('any_r127)])=?,
      ~max: option([ | `Int(int) | `Float(float)])=?,
      ~min: option([ | `Int(int) | `Float(float)])=?,
      ~name: option(string)=?,
      ~onChange: option((ReactEvent.Form.t, 'any_rdjl) => unit)=?,
      ~onChangeCommitted: option((Js.t({..}), 'any_rqci) => unit)=?,
