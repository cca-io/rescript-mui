...
external makePropsMui:
  (
    ...
    ~transitionDuration: 'union_rg4x=?,
    ~variant: string=?,
    ~key: string=?,
    ~ref: ReactDOMRe.domRef=?,
    ~anchorOrigin: 'any_rt7a=?,
    ~anchorPosition: 'any_r0sy=?,
    ~anchorReference: string=?,
    ~className: string=?,
    ~container: 'union_r0sc=?,
    ~elevation: 'number_d=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_r=?,
    ~transformOrigin: 'any_r36t=?,
    ~_TransitionComponent: 'union_r783=?,
