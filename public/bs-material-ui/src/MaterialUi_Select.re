...
external makePropsMui:
  (
    ...
    ~autoWidth: bool=?,
    ~children: 'children=?,
    ~defaultValue: 'any_rf2y=?,
    ~displayEmpty: bool=?,
    ~_IconComponent: 'union_rwbl=?,
    ~id: string=?,
    ~input: React.element=?,
    ~inputProps: Js.t({..})=?,
    ~label: React.element=?,
...
let makeProps =
    (
      ...
      ~open_: option(bool)=?,
      ~renderValue: option('any_rqqe => 'any_rpa8)=?,
