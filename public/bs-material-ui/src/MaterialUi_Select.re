...
external makePropsMui:
  (
    ...
    ~autoWidth: bool=?,
    ~children: 'children=?,
    ~defaultValue: 'any_runf=?,
    ~displayEmpty: bool=?,
    ~_IconComponent: 'union_rvzy=?,
    ~id: string=?,
    ~input: React.element=?,
    ~inputProps: Js.t({..})=?,
    ~label: React.element=?,
...
let makeProps =
    (
      ...
      ~open_: option(bool)=?,
      ~renderValue: option('any_rg80 => 'any_rdao)=?,
