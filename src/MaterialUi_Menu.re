...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_r69k=?,
    ~action: 'any_rxiq=?,
    ~anchorOrigin: 'any_rnzh=?,
    ~anchorPosition: 'any_r254=?,
    ~anchorReference: string=?,
    ~container: 'union_rpe8=?,
    ~elevation: 'number_g=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_c=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_rs6i=?,
    ~_TransitionComponent: 'union_rgk7=?,
