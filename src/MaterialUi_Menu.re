...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_rgri=?,
    ~action: 'any_rn2p=?,
    ~anchorOrigin: 'any_rqte=?,
    ~anchorPosition: 'any_rbxo=?,
    ~anchorReference: string=?,
    ~container: 'union_rr22=?,
    ~elevation: 'number_3=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_4=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_rkx7=?,
    ~_TransitionComponent: 'union_r9pk=?,
