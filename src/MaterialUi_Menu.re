...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_r7v0=?,
    ~action: 'any_rmjf=?,
    ~anchorOrigin: 'any_rtw6=?,
    ~anchorPosition: 'any_rn9m=?,
    ~anchorReference: string=?,
    ~container: 'union_rex3=?,
    ~elevation: 'number_6=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_2=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_rjen=?,
    ~_TransitionComponent: 'union_rszf=?,
