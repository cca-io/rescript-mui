...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_rp05=?,
    ~action: 'any_rry6=?,
    ~anchorOrigin: 'any_r9wv=?,
    ~anchorPosition: 'any_rbad=?,
    ~anchorReference: string=?,
    ~container: 'union_rwm7=?,
    ~elevation: 'number_q=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_z=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_rxs7=?,
    ~_TransitionComponent: 'union_rjem=?,
