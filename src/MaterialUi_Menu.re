...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_rfnx=?,
    ~action: 'any_rvvz=?,
    ~anchorOrigin: 'any_ry9z=?,
    ~anchorPosition: 'any_ris1=?,
    ~anchorReference: string=?,
    ~container: 'union_rote=?,
    ~elevation: 'number_8=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_o=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_ruuo=?,
    ~_TransitionComponent: 'union_rh21=?,
