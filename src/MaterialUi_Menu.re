...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_r5u8=?,
    ~action: 'any_re4q=?,
    ~anchorOrigin: 'any_rbkf=?,
    ~anchorPosition: 'any_rut8=?,
    ~anchorReference: string=?,
    ~container: 'union_rqpe=?,
    ~elevation: 'number_p=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_v=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_rifv=?,
    ~_TransitionComponent: 'union_rorm=?,
