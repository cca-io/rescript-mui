...
external makeProps:
  (
    ...
    ~theme: Js.t({..})=?,
    ~transitionDuration: 'union_ryvw=?,
    ~action: 'any_r33a=?,
    ~anchorOrigin: 'any_rtxd=?,
    ~anchorPosition: 'any_rrx0=?,
    ~anchorReference: string=?,
    ~container: 'union_rlhy=?,
    ~elevation: 'number_3=?,
    ~getContentAnchorEl: 'genericCallback=?,
    ~marginThreshold: 'number_2=?,
    ~_ModalClasses: Js.t({..})=?,
    ~role: string=?,
    ~transformOrigin: 'any_rdtt=?,
    ~_TransitionComponent: 'union_rmli=?,
