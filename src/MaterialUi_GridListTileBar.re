...
| ActionIconActionPosLeft(className) => Js.Dict.set(obj, to_string(classType), className)
                            };
                            obj
                        },
                    );
            ...
            [@bs.obj] external makeProps : (~actionIcon: ReasonReact.reactElement=?,
...
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/GridListTileBar/GridListTileBar"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
...
~classes: option(Classes.t)=?,~style: option(ReactDOMRe.Style.t)=?,
                children
                    ...
                    ~reactClass,
                    ~props=makeProps(~actionIcon=?actionIcon,
~actionPosition=?actionPosition |. Belt.Option.map((v => actionPositionToJs(v))),
~className=?className,
~subtitle=?subtitle,
~title=?title,
