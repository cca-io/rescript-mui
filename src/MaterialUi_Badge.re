...
| ColorError(className) => Js.Dict.set(obj, to_string(classType), className)
            ...
            [@bs.obj] external makeProps : (~badgeContent: ReasonReact.reactElement,
~className: string=?,
~color: string=?,
~component: 'union_rbsj=?,
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/Badge/Badge"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
...
~classes: option(Classes.t)=?,~style: option(ReactDOMRe.Style.t)=?,
                children
                    ...
                    ~reactClass,
                    ~props=makeProps(~badgeContent=badgeContent,
~className=?className,
