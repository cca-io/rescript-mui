...
external makeProps :
  (
    ...
    ~onOpen: 'any_rvo7=?,
    ~_open: bool=?,
    ~renderValue: 'any_rq5p=?,
    ~_SelectDisplayProps: Js.t({..})=?,
    ~value: 'union_repb=?,
    ~autoComplete: string=?,
    ~autoFocus: bool=?,
    ~className: string=?,
    ~defaultValue: 'union_rppl=?,
    ~disabled: bool=?,
...
let make =
    (
      ...
      ~open_: option(bool)=?,
      ~renderValue: option('any_rw3k => ReasonReact.reactElement)=?,
      ...
      ~id: option(string)=?,
      ~inputComponent: option('any_rnvb)=?,
      ~inputRef:
         option(
