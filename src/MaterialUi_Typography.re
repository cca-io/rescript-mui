...
| ColorError(className) => Js.Dict.set(obj, to_string(classType), className)
            ...
            [@bs.obj] external makeProps : (~align: string=?,
~className: string=?,
~color: string=?,
~component: 'union_r94w=?,
~gutterBottom: bool=?,
...
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/Typography/Typography"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
                ~align: option(align)=?,
...
~classes: option(Classes.t)=?,~style: option(ReactDOMRe.Style.t)=?,
                children
                    ...
                    ~reactClass,
                    ~props=makeProps(~align=?align |. Belt.Option.map((v => alignToJs(v))),
~className=?className,
