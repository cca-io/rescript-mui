...
| Grid_Xs_12(className) => Js.Dict.set(obj, to_string(classType), className)
            ...
            [@bs.obj] external makeProps : (~alignContent: string=?,
~alignItems: string=?,
~className: string=?,
~component: 'union_rfn1=?,
~container: bool=?,
~direction: string=?,
~item: bool=?,
~justify: string=?,
...
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/Grid/Grid"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
...
~classes: option(Classes.t)=?,~style: option(ReactDOMRe.Style.t)=?,
                children
                    ...
                    ~reactClass,
                    ~props=makeProps(~alignContent=?alignContent |. Belt.Option.map((v => alignContentToJs(v))),
