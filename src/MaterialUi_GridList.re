            ...
            [@bs.obj] external makeProps : (~cellHeight: 'union_rr10=?,
~className: string=?,
~cols: 'number_9=?,
~component: 'union_rk4z=?,
~spacing: 'number_u=?,
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/GridList/GridList"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
                ~cellHeight: option([ | `Int(int) | `Float(float) | `Enum(cellHeight_enum)  ])=?,
...
~classes: option(Classes.t)=?,~style: option(ReactDOMRe.Style.t)=?,
                children
                    ...
                    ~reactClass,
                    ~props=makeProps(~cellHeight=?
                    cellHeight |. Belt.Option.map((v) => switch v {
                        | `Enum(v) => MaterialUi_Helpers.unwrapValue(`String(cellHeight_enumToJs(v)))
...
~spacing=?spacing |. Belt.Option.map((v) => MaterialUi_Helpers.unwrapValue(v)),
