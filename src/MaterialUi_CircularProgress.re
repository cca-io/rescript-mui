...
| CircleIndeterminate(className) => Js.Dict.set(obj, to_string(classType), className)
                            };
                            obj
                        },
                    );
...
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/CircularProgress/CircularProgress"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
...
~thickness=?thickness |. Belt.Option.map((v) => MaterialUi_Helpers.unwrapValue(v)),
