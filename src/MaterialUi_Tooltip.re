...
| [@bs.as "top"] `Top
                ...
                module Classes = {
                    type classesType = | Popper(string)
| Open(string)
| Tooltip(string)
| Touch(string)
...
| TooltipPlacementBottom(className) => Js.Dict.set(obj, to_string(classType), className)
                            };
                            obj
                        },
                    );
...
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/Tooltip/Tooltip"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
...
~enterDelay=?enterDelay |. Belt.Option.map((v) => MaterialUi_Helpers.unwrapValue(v)),
~enterTouchDelay=?enterTouchDelay |. Belt.Option.map((v) => MaterialUi_Helpers.unwrapValue(v)),
~id=?id,
~leaveDelay=?leaveDelay |. Belt.Option.map((v) => MaterialUi_Helpers.unwrapValue(v)),
~leaveTouchDelay=?leaveTouchDelay |. Belt.Option.map((v) => MaterialUi_Helpers.unwrapValue(v)),
~onClose=?onClose,
~onOpen=?onOpen,
~_open=?open_,
~placement=?placement |. Belt.Option.map((v => placementToJs(v))),
~_PopperProps=?_PopperProps,
~theme=?theme,
~title=title,
