...
| Light(className) => Js.Dict.set(obj, to_string(classType), className)
            ...
            [@bs.obj] external makeProps : (~absolute: bool=?,
~className: string=?,
~component: 'union_rhix=?,
~inset: bool=?,
~light: bool=?,
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/Divider/Divider"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
...
~classes: option(Classes.t)=?,~style: option(ReactDOMRe.Style.t)=?,
                children
                    ...
                    ~reactClass,
                    ~props=makeProps(~absolute=?absolute,
~className=?className,
