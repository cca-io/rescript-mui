...
| Disabled(className)
| Error(className)
| Asterisk(className) => Js.Dict.set(obj, to_string(classType), className)
                            };
                            obj
                        },
                    );
...
~component: 'union_rxbd=?,
~disabled: bool=?,
~error: bool=?,
~focused: bool=?,
~required: bool=?,
~classes: Js.Dict.t(string)=?,~style: ReactDOMRe.Style.t=?, unit) => _ = "";
            [@bs.module "@material-ui/core/FormLabel/FormLabel"] external reactClass : ReasonReact.reactClass = "default";
            let make = (
